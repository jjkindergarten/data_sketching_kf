...
from src.data_sketching_kalman_filter.shared.sketching_projection import count_sketching
from src.data_sketching_kalman_filter.shared.kalman_filter import *
from time import time


def count_kf_mse(y, X, Q, R, F, theta_predict, P_predict, d, approx_K = True, sketch_size = 0.2, iteration_step = 100):
...
if __name__ == "__main__":
    ...
    for mm in range(10):
        ...
        for i in range(N):
            ...
            theta_predict, P_predict = count_kf_mse(y_set[i], X_set[i], Q, R, F, theta_predict, P_predict, d,
                                                   approx_K = False, sketch_size = 0.05, iteration_step = 4)
            kf_end = time()
    ...
    # from src.data_sketching_kalman_filter.full_KF import full_kf_mse
    #
    # running_time = []
    # mse_set = []
    # P_predict = np.identity(p)
